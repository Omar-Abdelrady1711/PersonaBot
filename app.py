...
this still needs more modifications im aware
"""
...
h1 { font-family:'Syne',sans-serif !important; font-size:2.6rem !important;
     font-weight:800 !important; letter-spacing:-0.03em !important; margin-bottom:0.2rem !important; }
h3 { font-family:'Syne',sans-serif !important; font-size:1.05rem !important;
     font-weight:700 !important; margin:1.4rem 0 0.6rem !important; }

/* ── chat bubbles ── */
.bubble-user {
    background:#1a2035; border-left:3px solid #4f8ef7;
    padding:14px 18px; border-radius:8px; margin:10px 0 4px;
    font-size:1rem; line-height:1.7; color:#dce3f0;
}
.bubble-bot {
    background:#111825; border-left:3px solid #3dd68c;
...
.lbl-you  { color:#4f8ef7; }
.lbl-bot  { color:#3dd68c; }

/* ── chips ── */
.chip {
    font-family:'IBM Plex Mono',monospace; font-size:0.72rem;
    padding:2px 9px; border-radius:4px; font-weight:500;
}
...
/* ── correction box ── */
.corr-box {
    background:#180808; border-left:3px solid #e05555;
    padding:10px 14px; border-radius:6px; margin-top:8px;
    font-family:'IBM Plex Mono',monospace; font-size:0.8rem;
    color:#cc5555; line-height:1.5;
}

/* ── trait bar ── */
.trow { display:flex; align-items:center; gap:10px; margin:6px 0; }
.tname { font-family:'IBM Plex Mono',monospace; font-size:0.72rem;
         color:#8898bb; width:82px; flex-shrink:0; }
...
/* ── stat card ── */
.scard { background:#0e1525; border:1px solid #1c2a40; border-radius:8px;
         padding:16px 20px; margin:6px 0; }
.sval  { font-family:'Syne',sans-serif; font-size:2.2rem; font-weight:800; line-height:1; }
.slbl  { font-family:'IBM Plex Mono',monospace; font-size:0.65rem;
         color:#5a6888; letter-spacing:0.08em; text-transform:uppercase; margin-top:4px; }

/* ── compare card ── */
.ccard { background:#0e1525; border:1px solid #1c2a40; border-radius:8px;
         padding:18px 20px; height:100%; }
.clabel { font-family:'IBM Plex Mono',monospace; font-size:0.72rem;
          letter-spacing:0.1em; text-transform:uppercase; color:#5a6888;
          margin-bottom:12px; display:flex; align-items:center; gap:8px; }
.ctext  { font-size:0.97rem; line-height:1.75; color:#c8d4e8; }

/* ── section header ── */
.sec { font-family:'IBM Plex Mono',monospace; font-size:0.68rem;
       letter-spacing:0.14em; text-transform:uppercase; color:#3a4a60;
...
# ── Mock helpers ──────────────────────────────────────────────────────────────
_MOCK = {
    "baseline":           ["That's a good question. Let me help you with that. Here's some information that might be useful to you.",
                           "Sure, I can help with this. Here is some relevant information on the topic."],
    "formal_expert":      ["The subject matter you have raised pertains to a well-documented domain. "
                           "Permit me to elucidate the underlying mechanisms with precision and rigour."],
    "friendly_support":   ["Oh I totally get that — sounds like so much to deal with! 😊 "
                           "You're absolutely not alone in feeling this way. What's been the hardest part for you?"],
    ...
    "analytical_assistant":["Parsing your query. Three primary factors identified. "
                            "Confidence interval: 87–93%. Recommendation follows logically from the data."],
}
...
def _mock_detect(msg):
    m = msg.lower()
    if any(w in m for w in ["stress","overwhelm","help","feel","scared","sad","worried"]):
        return Persona(name="detected_user",formality=0.2,empathy=0.85,technical_depth=0.15,
                       verbosity=0.4,assertiveness=0.25,humor=0.15,politeness=0.85,curiosity=0.65,tone="warm")
    if any(w in m for w in ["technical","explain","architect","analys","formal","precise","research"]):
        return Persona(name="detected_user",formality=0.85,empathy=0.1,technical_depth=0.85,
                       verbosity=0.7,assertiveness=0.8,humor=0.0,politeness=0.5,curiosity=0.3,tone="authoritative")
    return Persona(name="detected_user",formality=0.4,empathy=0.5,technical_depth=0.4,
                   verbosity=0.45,assertiveness=0.45,humor=0.25,politeness=0.65,curiosity=0.5,tone="neutral")

def _mock_match(d):
    best,bd = None,float("inf")
    for name,p in PRESETS.items():
        dist=math.sqrt(sum((getattr(d,t)-getattr(p,t))**2 for t in TRAITS))
        if dist<bd: bd,best=dist,name
    return best,PRESETS[best]

def _mock_smooth(cur,det,a=0.3):
    b=lambda x,y:round((1-a)*x+a*y,4)
    return Persona(name=cur.name,formality=b(cur.formality,det.formality),
                   empathy=b(cur.empathy,det.empathy),technical_depth=b(cur.technical_depth,det.technical_depth),
                   verbosity=b(cur.verbosity,det.verbosity),assertiveness=b(cur.assertiveness,det.assertiveness),
                   humor=b(cur.humor,det.humor),politeness=b(cur.politeness,det.politeness),
                   curiosity=b(cur.curiosity,det.curiosity),tone=det.tone)
...
def _mock_resp(msg,hist,persona,correction,cond):
    ...
    if correction and cond=="adaptive_reinforce": bot="[Recalibrated] "+bot
...
run_detect   = lambda m:   _mock_detect(m)   if MOCK_MODE else detect_tone(m)
run_match    = lambda d:   _mock_match(d)    if MOCK_MODE else match_preset(d)
run_smooth   = lambda c,d: _mock_smooth(c,d) if MOCK_MODE else smooth_persona(c,d)
run_resp     = lambda *a:  _mock_resp(*a)    if MOCK_MODE else get_bot_response(*a)


# ── Session state ─────────────────────────────────────────────────────────────
for k,v in dict(hist_base=[],hist_adap=[],hist_reinf=[],
                active_persona=PRESETS["friendly_support"],
                pending_correction=None,turn_count=0).items():
...
def radar(scores,persona,sz=240):
    ...
    return (f'<svg width="{sz}" height="{sz}" xmlns="http://www.w3.org/2000/svg" '
...
# ══════════════════════════════════════════════════════════════════════════════
with st.sidebar:
    st.markdown('<div class="sec">◈ &nbsp;Active Persona</div>', unsafe_allow_html=True)
    ap=st.session_state.active_persona
    st.markdown(f'<span class="chip chip-blue" style="font-size:0.82rem;padding:4px 12px">⬡ &nbsp;{ap.name}</span>', unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown(trait_bars(ap), unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown('<div class="sec">Condition</div>', unsafe_allow_html=True)
    condition=st.radio("",["baseline","adaptive","adaptive_reinforce"],
        format_func=lambda x:{"baseline":"① Baseline","adaptive":"② Adaptive",
                               "adaptive_reinforce":"③ Adaptive + Reinforce"}[x],
    ...
    st.caption(f"Fires when alignment < {threshold:.2f}")

    st.markdown("<br>", unsafe_allow_html=True)
    if st.button("↺  Reset conversation", use_container_width=True):
        for k in ["hist_base","hist_adap","hist_reinf"]:
            st.session_state[k]=[]
        st.session_state.active_persona=PRESETS["friendly_support"]
...
st.caption("NLP Course Project  ·  Live Demo Interface  ·  April 2026")
...
# ── Chat ──────────────────────────────────────────────────────────────────────
with chat_col:
    clabels={"baseline":"① Baseline","adaptive":"② Adaptive","adaptive_reinforce":"③ Adaptive + Reinforce"}
    ...
    for turn in history:
        ...
        preset_chip = f'<span class="chip chip-blue">⬡ {preset}</span>' if preset!="none" \
                      else '<span class="chip chip-grey">baseline</span>'
        ...
        corr_html   = f'<div class="corr-box">⚡ Correction → {turn["correction"]}</div>' if turn.get("correction") else ""
    ...
    with st.form("chat_form", clear_on_submit=True):
        ...
        with c1: user_input=st.text_input("","",placeholder="Type your message…",label_visibility="collapsed")
        with c2: submitted=st.form_submit_button("Send →", use_container_width=True)

    if submitted and user_input.strip():
        with st.spinner("Thinking…"):
            if condition!="baseline":
                detected=run_detect(user_input)
                pname,preset=run_match(detected)
                sm=run_smooth(st.session_state.active_persona,preset)
                sm=Persona(name=pname,formality=sm.formality,empathy=sm.empathy,
                           technical_depth=sm.technical_depth,verbosity=sm.verbosity,
                           assertiveness=sm.assertiveness,humor=sm.humor,
                           politeness=sm.politeness,curiosity=sm.curiosity,tone=sm.tone)
                st.session_state.active_persona=sm
                ap=sm
            else:
                ap=None

            pending=st.session_state.pending_correction if condition=="adaptive_reinforce" else None
            turn=run_resp(user_input,history,ap,pending,condition)
...
with panel_col:
    ...
    if history:
        ...
        persona=last.get("detected_persona") or st.session_state.active_persona
        if scores and persona:
            st.markdown(radar(scores,persona), unsafe_allow_html=True)
    else:
        st.caption("Send a message to see the radar.")

    if history:
        last=history[-1]; scores=last.get("alignment_score") or {}
        if scores:
            st.markdown("<br>", unsafe_allow_html=True)
...
st.caption("Run the same message through all three conditions at once — great for the demo.")

with st.form("cmp_form"):
    cc1,cc2=st.columns([4,1])
    with cc1: cmp_in=st.text_input("","",placeholder="e.g.  I'm really stressed about my exam tomorrow",label_visibility="collapsed")
    with cc2: run_cmp=st.form_submit_button("Run All →", use_container_width=True)

if run_cmp and cmp_in.strip():
    cols=st.columns(3,gap="medium")
    configs=[("baseline","① Baseline","#4a5570"),("adaptive","② Adaptive","#4f8ef7"),
             ("adaptive_reinforce","③ Adaptive + Reinforce","#3dd68c")]
    for col,(cond,label,color) in zip(cols,configs):
        p=None
        if cond!="baseline":
            det=run_detect(cmp_in); _,p=run_match(det)
        t=run_resp(cmp_in,[],p,None,cond)
        if cond!="baseline": t=score_and_reinforce(t,threshold=threshold)
